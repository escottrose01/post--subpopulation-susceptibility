 ...
 */
export function createD3RangeSlider(rangeMin, rangeMax, containerSelector, playButton) {
  "use strict";
  ...
  var dragResizeW = d3
    .drag()
    .on("start", function (event) {
      startX = d3.pointer(event, handleW.node())[0];
      event.sourceEvent.stopPropagation();
      resumePlaying = playing;
      playing = false;
    })
    ...
    .on("drag", function (event) {
      var dx = d3.pointer(event, handleW.node())[0] - startX;
      if (dx == 0) return;
  ...
  sliderBox.on("mousedown", function (event) {
    ...
    var x = d3.pointer(event, sliderBox.node())[0];
    var props = {};
